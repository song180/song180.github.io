...
/**
	...
	@param
		...
		value 样式值
*/
function css1(elem,param1,param2){
	//如果是获取样式的值
...
/**
	...
	@param 
		...
		isCapture 是否捕捉
*/
function addEventListener1(elem,type,fn,isCapture){
...
/*
	...
	@param
		...
		expires 存储的期限 (正整数)
*/
function setCookie1(cname,cvalue,expires){
	if(arguments.length == 2){
...
/*
	...
	@param
		...
		className 要添加的class
*/
function addClass1(elem,className){
...
/*
	...
	@param
		...
		className 要去除的class
*/
function removeClass1(elem,className){
	//将元素的class按照空格进行分割
	var c = elem.className; // elem.getAttribute('class');
	var classArr = c.split(' ');
	var cname = [];
...
function ajax1(json){
	...
	xhr.onreadystatechange1 = function(){
		if(xhr.readyState == 4){
			//调用success方法
...
/**
 	获取浏览器的可用宽度和高度
*/
function getClientWidth1(){
	return document.documentElement.clientWidth;
}
function getClientHeight1(){
...
function animate1(elem,data,time,fn){
	...
	for(var key in data){
		//获取初值
		var start = parseFloat(css1(elem,key));
		var json = {
			start: start,
			end: data[key], 
